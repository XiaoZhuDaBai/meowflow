 ...
 */
@Slf4j
public class WxworkSender implements IntegrationSender {

    private final IntegrationConfig config;
    private final RestTemplate restTemplate;

    public WxworkSender(IntegrationConfig config) {
        this.config = config;
        ...
        try {
            ...
            if (response.getBody() != null && 0 == (Integer) response.getBody().get("errcode")) {
                return SendResult.success(
                        String.valueOf(response.getBody().get("msgid")),
                        System.currentTimeMillis() - start);
            }
            return SendResult.failure("企业微信发送失败: " + response.getBody(), System.currentTimeMillis() - start);
        } catch (Exception e) {
            log.error("Wxwork send error", e);
    ...
    public String getType() {
        return IntegrationConfig.TYPE_WXWORK;
    }
}
